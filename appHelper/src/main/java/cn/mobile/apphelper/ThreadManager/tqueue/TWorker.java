 ...
 * @date:2017年4月26日
 */
public class TWorker<T> implements Runnable{
	...
	public void run(){
		...
		if (queue != null && queue.isWorking()){
			if (isUIThread) {
				...
				try {
					final T aa=  thing.doing(queue, queue.getWorkerArgument());
					queue.getUIHandler().post(new Runnable() {
						@Override
						public void run() {
							handlerSuccess(queue, aa);
						}
					});
	...
	public void handlerSuccess(final TQueue queue,final T str){
		Ceased error = queue.getCeased();
		if (error != null) {
			error.doing(null,str);
		}
	}
